...
abstract class GenerateModuleTitlesTask : DefaultTask() {
    ...
    /**
     * Built line by line rather than from a `trimIndent()` template.
     *
     * The template form worked for one module and broke silently at two.
     * `trimIndent` computes the common indent across the string *after*
     * interpolation, and a multi-line value pasted in at one indentation level
     * carries its own for every line after the first — so the common indent
     * collapsed to the entries' own, every template line kept the difference, and
     * the `<?xml?>` declaration ended up several columns in. XML forbids anything
     * before it, so AAPT failed with "The processing instruction target matching
     * [xX][mM][lL] is not allowed" — a message that says nothing about indentation.
     */
    @TaskAction
    fun generate() {
        val xml = buildString {
            appendLine("""<?xml version="1.0" encoding="utf-8"?>""")
            appendLine("<!-- Generated by omnideck.android.application (OD-301) — DO NOT EDIT. -->")
            appendLine("<resources>")
            splitNames.get().forEach { splitName ->
                appendLine("""    <string name="${moduleTitleResource(splitName)}">${splitName.humanise()}</string>""")
            }
            appendLine("</resources>")
        }
